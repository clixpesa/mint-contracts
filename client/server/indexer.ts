...
import { parseAbiItem, createPublicClient, http, Address, getContract, parseEther, formatUnits} from 'viem'
...
const unwatch = publicClient.watchEvent({
  ...
  args: {
    to: users.map((user) => user.address as Address) 
  },
  onLogs: logs => checkForOverdraft(logs)
})

async function checkForOverdraft(events: any){
  events.forEach(async (log: { args: { from: string; to: string, value: bigint}; }) => {
    if(log.args.from == overdraftAddress){
      console.log(`UsedOverdraft | ${log.args.to}`)
    } else {
      const cKESBalance: any = await localStableContract.read.balanceOf([log.args.to]);
      const cUSDBalance: any = await usdStableContract.read.balanceOf([log.args.to])
      ...
      if(cKESBalance > parseEther("1")) {
        await repayOverdraft({
          ...
          token: localStable,
          amount: overdraftDebt.amountDue > cKESBalance ? formatUnits(cKESBalance, 18) : formatUnits(overdraftDebt.amountDue, 18)
        })
      ...
      if(cUSDBalance > parseEther("0.01")) {
        await repayOverdraft({
          ...
          userAddress: log.args.to as Address,
          token: usdStable,
          amount: overdraftDebt.amountDue > Number(cUSDBalance) * 128 ? formatUnits(cUSDBalance, 18) : formatUnits(BigInt((Number(overdraftDebt.amountDue)/128)), 18)
        })
