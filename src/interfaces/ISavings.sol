...
interface ISavings {
  ...
  // Events
  event Created(address indexed user, bytes8 indexed spaceId, SavingType indexed type);
  ...
  function create(string memory _name, , uint256 _deadline, SavingType type) external returns (bytes8 spaceId);

  // Create a challenge saving space
  function createChallenge(string memory _name, uint256 _baseAmount, uint256 _duration, uint256 _target, ChallengPref _preference) external returns (bytes8 spaceId);
  ...
  function editChallenge(bytes8 id, string memory _name, uint256 _baseAmount, uint256 _duration, uint256 _target, ChallengPref _preference) external;
